...
import type { TeacherAvatarSvgProps } from './teacher.types';

export const TeacherAvatarSvg = memo(function TeacherAvatarSvg({
  isPlaying = false,
  isWriting = false,
  className = '',
}: TeacherAvatarSvgProps) {
  return (
    <svg
      viewBox="0 0 120 220"
      width="120"
      ...
      className={`relative z-10 ${isPlaying ? 'animate-teacher-idle' : ''} ${className}`}
      aria-label="Prof. Gemini Avatar"
    >
      {/* Graduation cap */}
      ...
      {/* Chalk in left hand when writing */}
      {isWriting && (
...
TeacherAvatarSvg.displayName = 'TeacherAvatarSvg';
